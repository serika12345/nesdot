...
const DEV_SERVER_URLS = [`http://localhost:${DEV_SERVER_PORT}/`, `http://127.0.0.1:${DEV_SERVER_PORT}/`, `http://[::1]:${DEV_SERVER_PORT}/`];
...
async function inspectUrl(url) {
    ...
    try {
        ...
        const isExpectedServer = EXPECTED_MARKERS.some((marker) => html.includes(marker));
...
if (existingServer.isRunning && !existingServer.isExpectedServer) {
    console.error(`[tauri] Port ${DEV_SERVER_PORT} is already in use by another service. Stop it or run nesdot on a different port.`);
    process.exit(1);
}
