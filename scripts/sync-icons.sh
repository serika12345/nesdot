...
if [[ "$(uname -s)" == "Darwin" ]] && command -v sips >/dev/null && command -v iconutil >/dev/null; then
  ...
  if ! run_magick \
    ...
    "$SOURCE_ICON" \
    -resize 560x560 \
    "$WORK_TMP_DIR/logo.png"; then
    echo "Skipping macOS-specific icon tile generation because neither nix nor ImageMagick is available"
    echo "Keeping the default icon.icns generated by tauri icon"
  else
    run_magick \
