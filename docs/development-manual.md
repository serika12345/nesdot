...
1. [`README.md`](../README.md)
   プロダクト概要、起動方法、検証コマンド、リリースの入口があります。
...
5. [`src/main.tsx`](../src/main.tsx)
   アプリ起動時の provider 構成と CSP nonce の扱いが分かります。
...
詳細なサポート範囲は [`README.md`](../README.md) の WSL 節にまとまっています。
...
- CSP nonce を `getCspNonce()` から取得
- Emotion cache を nonce 付きで作成
- MUI theme を `ThemeProvider` に注入
- `App` を mount

このため、CSP や style 注入まわりを触るときは `main.tsx` と `src-tauri/tauri.conf.json` を対で確認してください。
...
### 8.2 TypeScript / React / MUI の重要ルール
...
- MUI の見た目は theme / shared wrapper / `styled(...)` を優先し、`sx` は小さな一時的調整に限定する
...
- 必要に応じて `src/presentation/theme.ts`

方針:

- 画面構成は MUI primitive と shared component で表す
- 大きな `sx` オブジェクトを足し始めたら抽出を検討する
...
`verify:security` は単なる依存監査ではありません。README にもあるとおり、次のような境界を機械的に見ています。
