...
describe("makeTile", () => {
    it("creates an 8-pixel wide tile filled with the requested value", () => {
        ...
        expect(tile.pixels.every((row) => row.every((value) => value === 2))).toBe(true);
    });
});
...
describe("resizeTileND", () => {
    it("preserves hidden pixels when shrinking and expanding with the same anchor", () => {
        const shrunk = resizeTileND(createTallTile(), 8, 8, { anchor: "top-left", fill: 0 });
        const expanded = resizeTileND(shrunk, 8, 16, { anchor: "top-left", fill: 0 });

        expect(expanded.paletteIndex).toBe(2);
        expect(expanded.pixels.slice(0, 8).every((row) => row.every((value) => value === 1))).toBe(true);
        expect(expanded.pixels.slice(8).every((row) => row.every((value) => value === 2))).toBe(true);
    });
