...
人間向けの詳細な開発手順、アーキテクチャ、変更種別ごとの作業ガイドは [`docs/development-manual.md`](./docs/development-manual.md) を参照してください。

リリース運用の runbook は [`docs/release-checklist.md`](./docs/release-checklist.md) にあります。
...
VS Code では `Terminal: Run Task` から `Verify Licenses`, `Verify Security`, `Verify System Libraries`, `Verify CVE`, `Verify`, `Verify UI`, `Verify Full`, `Verify Rust`, `Verify CI`, `Run Unit Tests`, `Run Console E2E`, `Run Full E2E`, `Format Check` を直接実行できます。
desktop release は `Release Desktop Dry Run` と `Deploy Desktop Release` から開始できます。どちらも VS Code の quick pick で `patch / minor / major` を選べます。
...
`develop -> main` マージ、タグ作成 (`vX.Y.Z`)、desktop release workflow 起動までを自動化できます。`main` への push により Pages deploy も自動で走ります。

VS Code から実行する場合は `Terminal: Run Task` で次の順に進めます。

- `Verify CI`
- `Release Desktop Dry Run`
- `Deploy Desktop Release`

詳細な runbook は [`docs/release-checklist.md`](./docs/release-checklist.md) を参照してください。
...
鍵に password を設定している場合は、同じ値を `TAURI_SIGNING_PRIVATE_KEY_PASSWORD` にも登録してください。
